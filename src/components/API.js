...
function API() {
  ...
  useEffect(() => {
    loadMovies(); // Load trending movies by default
      ...
      {movieList.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <div style={{ marginTop: "20px" }}>
            <button onClick={handlePrevious} disabled={currentIndex === 0} style={{ marginRight: "10px", padding: "10px 20px", backgroundColor: "rgba(255, 0, 0, 0.7)", color: "white", border: "none", borderRadius: "5px" }}>
              Previous
            </button>
            <button onClick={handleNext} disabled={currentIndex >= movieList.length - 1} style={{ padding: "10px 20px", backgroundColor: "rgba(255, 0, 0, 0.7)", color: "white", border: "none", borderRadius: "5px" }}>
              Next
            </button>
          </div>
        </div>
